...
dflts = {'.', ... %default_path.raw_path,...        
        ...
        fullfile(mortarpath,'resources', 'L1000_poolinfo.txt'),...
        fullfile(mortarpath,'resources', 'L1000_bead_batch.txt'),...
        '2.2',...
        false,...
        'DUO52HI53LO'};
...
%     if args.verify && ...
%             ~isequal(length(res.bset), length(intersect(res.bset, smap_bset)))
%         error('Platename:%s does not match beadset(s) specified in map file', p);
%     end
    ...
    res.missing_analytes = tok(find(~cellfun(@isempty, regexpi(tok, match))))';
