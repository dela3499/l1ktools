...
%                     % one pool, one bset
%                     % load csv datasets
%                     ds = parse_csv0(plateinfo.csv_path);
%                     
%                     %Bead count info
%                     dscnt = parse_csv0(plateinfo.csv_path, 'type', 'Count');
%                     mean_count = mean(dscnt.ge(11:end,:))';
%                     cv_count = 100*std(dscnt.ge(11:end,:))' ./ max(mean_count,1);
%                     
%                     [nGene, nSample] = size(ds.ge);
%                     [wells, ~] = get_wellinfo(ds.sid);
%                     vals = welldict.values(wells);
%                     ds.well = wells;
%                     %ds.sid = cellfun(@(x) x.det_name, vals, 'uniformoutput', false);
%                     % descriptor dict for each well (one dict per well)
%                     well_desc = cellfun(@(x) x.sm_desc, vals, 'uniformoutput', false);
%                     % merge into one dictionary
%                     ds.sdesc = merge_celldict(well_desc);
%                     %add bead count stats
%                     ds.sdesc('count_mean')  = num2cellstr(mean_count, 'precision', 0);
%                     ds.sdesc('count_cv') = num2cellstr(cv_count, 'precision', 0);
%                     
%                     ds.sid = cellfun(@(x) x.prof_name, vals, 'uniformoutput', false);
%                     % beadset info
%                     ds.bset = cellfun(@(x) x.bset, vals, 'uniformoutput', false);
%                     % feature mapping
%                     [ds.gn, ds.gd, gidx] = ...
%                         map_features(fmap, ds.gn, ...
%                         welldict(wells{1}).pool, plateinfo.bset{1},...
%                         'bset_revision', plateinfo.bset_revision, ...
%                         varargin{:});
%                     % keep only features that could be mapped
%                     ds.ge = ds.ge(gidx,:);
%                     
%                     %count info
%                     dscnt.sid = ds.sid;
%                     dscnt.gn = ds.gn;
%                     dscnt.gd = ds.gd;
%                     dscnt.ge = dscnt.ge(gidx, :);
%                     
%                     % create GEX file
%                     mkgct0(fullfile(plateinfo.plate_path,...
